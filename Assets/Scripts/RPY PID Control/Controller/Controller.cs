using UnityEngine;
using System.Collections;

public class Controller : MonoBehaviour
{
    public bool isAgentControl = false;
    public float Throttle = 0.0f;
    public float Yaw = 0.0f;
    public float Pitch = 0.0f;
    public float Roll = 0.0f;

    public enum ThrottleMode { None, LockHeight };

    [Header("Throttle Command")]
    public string ThrottleCommand = "Throttle";
    public bool InvertThrottle = true;

    [Header("Yaw Command")]
    public string YawCommand = "Yaw";
    public bool InvertYaw = false;

    [Header("Pitch Command")]
    public string PitchCommand = "Pitch";
    public bool InvertPitch = true;

    [Header("Roll Command")]
    public string RollCommand = "Roll";
    public bool InvertRoll = true;

    void Update()
    {
        if (!isAgentControl)
        {
            ReadInput();
        }
    }

    private void ReadInput()
    {
        Throttle = Mathf.Clamp(Input.GetAxisRaw(ThrottleCommand) * (InvertThrottle ? -1 : 1), -1f, 1f);
        Yaw = Mathf.Clamp(Input.GetAxisRaw(YawCommand) * (InvertYaw ? -1 : 1), -1f, 1f);
        Pitch = Mathf.Clamp(Input.GetAxisRaw(PitchCommand) * (InvertPitch ? -1 : 1), -1f, 1f);
        Roll = Mathf.Clamp(Input.GetAxisRaw(RollCommand) * (InvertRoll ? -1 : 1), -1f, 1f);
    }

    public void InputAction(float throttle, float pitch, float roll, float yaw)
    {
        if (isAgentControl)
        {
            Throttle = Mathf.Clamp(throttle, -1f, 1f);
            Yaw = Mathf.Clamp(yaw, -1f, 1f);
            Pitch = Mathf.Clamp(pitch, -1f, 1f);
            Roll = Mathf.Clamp(roll, -1f, 1f);
        }
    }
}